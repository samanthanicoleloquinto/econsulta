...
// ✅ Auto-expire logic with error checking based on preferred_schedule for TB DOTS forms
$expire_sql = "  
    UPDATE tbdots   
    SET status = 'expired'   
    ...
    if ($id && in_array($action, ['save', 'reject'])) {  
        if ($action === 'save') {  
            if ($preferred_schedule && strtotime($preferred_schedule) > time()) {  
                $stmt = $conn->prepare("UPDATE tbdots SET preferred_schedule = ?, status = 'approved' WHERE id = ?");  
                $stmt->bind_param("si", $preferred_schedule, $id);  
            } else {  
                $stmt = $conn->prepare("UPDATE tbdots SET preferred_schedule = ? WHERE id = ?");  
                $stmt->bind_param("si", $preferred_schedule, $id);  
            }  
        } elseif ($action === 'reject') {  
            $stmt = $conn->prepare("UPDATE tbdots SET status = ? WHERE id = ?");  
...
<head>  
<meta charset="UTF-8" />  
<title>Admin - TB DOTS Requests</title>  
...
<div class="sidebar">  
  ...
  <a href="admin_freemedicine.php">Free Medicine</a>  
  ...
  <table>
    ...
    <?php if ($result && $result->num_rows > 0): ?>
      ...
      <tr>
        ...
        <td><?= htmlspecialchars($row['tb_case_number']) ?></td>
        ...
        <td>
          ...
          <?php else: ?>
            <form method="POST" action="admin_tbdots.php">
      ...
      <?php endwhile; ?>
    <?php else: ?>
      <tr><td colspan="6" class="no-results">No TB DOTS requests found.</td></tr>
    <?php endif; ?>
    </tbody>
  </table>
