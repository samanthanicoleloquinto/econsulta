    ...
    if ($id && in_array($action, ['save', 'reject'])) {  
        if ($action === 'save') {  
            if ($preferred_schedule && strtotime($preferred_schedule) > time()) {  
                $stmt = $conn->prepare("UPDATE family_planning SET preferred_schedule = ?, status = 'approved' WHERE id = ?");  
                $stmt->bind_param("si", $preferred_schedule, $id);  
            } else {  
                $stmt = $conn->prepare("UPDATE family_planning SET preferred_schedule = ? WHERE id = ?");  
                $stmt->bind_param("si", $preferred_schedule, $id);  
            }  
        } elseif ($action === 'reject') {  
            $stmt = $conn->prepare("UPDATE family_planning SET status = ? WHERE id = ?");  
...
<head>  
<meta charset="UTF-8" />  
<title>Admin - Family Planning Requests</title>  
...
<style>  
  * { box-sizing: border-box; }  
  ...
  <div class="main-content">
    <h2>Family Planning Requests</h2>
    ...
    <table>
      ...
      <?php if ($result && $result->num_rows > 0): ?>
        ...
        <tr>
          ...
          <td><?= htmlspecialchars($row['using_method']) ?></td>
    ...
    <?php else: ?>
      <form method="POST" action="admin_familyplan.php">
    ...
    <?php endif; ?>
        ...
        <?php endwhile; ?>
      <?php else: ?>
        <tr><td colspan="7" class="no-results">No family planning requests found.</td></tr>
      <?php endif; ?>
      </tbody>
    </table>
