...
// ✅ Auto-expire logic with error checking based on preferred_schedule for pregnancy forms
$expire_sql = "  
    UPDATE pregnancy_forms   
    SET status = 'expired'   
    ...
    if ($id && in_array($action, ['save', 'reject'])) {  
        if ($action === 'save') {  
            if ($preferred_schedule && strtotime($preferred_schedule) > time()) {  
                $stmt = $conn->prepare("UPDATE pregnancy_forms SET preferred_schedule = ?, status = 'approved' WHERE id = ?");  
                $stmt->bind_param("si", $preferred_schedule, $id);  
            } else {  
                $stmt = $conn->prepare("UPDATE pregnancy_forms SET preferred_schedule = ? WHERE id = ?");  
                $stmt->bind_param("si", $preferred_schedule, $id);  
            }  
        } elseif ($action === 'reject') {  
            $stmt = $conn->prepare("UPDATE pregnancy_forms SET status = ? WHERE id = ?");  
...
$query = "SELECT * FROM pregnancy_forms";  
$params = [];  
$types = '';  
...
<head>  
<meta charset="UTF-8" />  
<title>Admin - Pregnancy Form Requests</title>  
...
<div class="main-content">
  ...
  <table>
    ...
    <?php if ($result && $result->num_rows > 0): ?>
      ...
      <tr>
        ...
        <td><?= htmlspecialchars($row['pregnant']) ?></td>
  ...
  <?php else: ?>
    <form method="POST" action="admin_pregnant.php">
  ...
  <?php endif; ?>
      ...
      <?php endwhile; ?>
    <?php else: ?>
      <tr><td colspan="7" class="no-results">No pregnancy requests found.</td></tr>
    <?php endif; ?>
    </tbody>
  </table>
