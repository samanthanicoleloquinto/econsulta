...
// ✅ Auto-expire logic with error checking based on preferred_schedule for oral services
$expire_sql = "  
    UPDATE oral_services   
    SET status = 'expired'   
    ...
    if ($id && in_array($action, ['save', 'reject'])) {  
        if ($action === 'save') {  
            if ($preferred_schedule && strtotime($preferred_schedule) > time()) {  
                $stmt = $conn->prepare("UPDATE oral_services SET preferred_schedule = ?, status = 'approved' WHERE id = ?");  
                $stmt->bind_param("si", $preferred_schedule, $id);  
            } else {  
                $stmt = $conn->prepare("UPDATE oral_services SET preferred_schedule = ? WHERE id = ?");  
                $stmt->bind_param("si", $preferred_schedule, $id);  
            }  
        } elseif ($action === 'reject') {  
            $stmt = $conn->prepare("UPDATE oral_services SET status = ? WHERE id = ?");  
...
$query = "SELECT * FROM oral_services";  
$params = [];  
$types = '';  
...
<head>  
<meta charset="UTF-8" />  
<title>Admin - Oral Services Requests</title>  
  ...
  <table>
    ...
    <?php if ($result && $result->num_rows > 0): ?>
      ...
      <tr>
        ...
        <td><?= htmlspecialchars($row['oral_service']) ?></td>
        ...
        <td>
          ...
          <?php else: ?>
            <form method="POST" action="admin_toothextraction.php">
      ...
      <?php endwhile; ?>
    <?php else: ?>
      <tr><td colspan="7" class="no-results">No oral service requests found.</td></tr>
    <?php endif; ?>
    </tbody>
  </table>
